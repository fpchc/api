...
class Message(db.Model):
    ...
    is_connected_to_network = db.Column(db.Boolean, nullable=False, server_default=db.text("false"))
    network_message = db.Column(db.JSON, nullable=False)
...
class Advertising(db.Model):
    ...
    __table_args__ = (
        db.PrimaryKeyConstraint("id", name="advertising_pkey"),
    )

    id = db.Column(StringUUID, server_default=text("uuid_generate_v4()"), primary_key=True, nullable=False)
    weigh = db.Column(db.BigInteger, nullable=False)
    icon = db.Column(db.String(200), nullable=False)
    icon_type = db.Column(db.String(200), nullable=False)
    started_time = db.Column(db.DateTime, nullable=False)
    ended_time = db.Column(db.DateTime, nullable=False)
    status = db.Column(db.String(24), nullable=False, server_default=text("'normal'"))
